...
S_INFO = 7  # bit_rate, buffer_size, next_chunk_size, bandwidth_measurement(throughput and time), chunk_til_video_end, video_score
...
NN_MODEL = './results/HSDPA_nn_model_ep_99800.ckpt'
# NN_MODEL = None
    ...
    with tf.Session() as sess, open(LOG_FILE + '_test', 'wb') as test_log_file:
        ...
        while True:
            ...
            if epoch % MODEL_SAVE_INTERVAL == 0:
                ...
                save_path = saver.save(sess, SUMMARY_DIR + "/HSDPA_nn_model_ep_" +
                                       str(epoch) + ".ckpt")
            ...
            """
            if (epoch >= 1) and (epoch <= 10000):
                a3c.ENTROPY_WEIGHT = 5

            elif (epoch > 10000) and (epoch <= 25000):
                a3c.ENTROPY_WEIGHT = 1

            elif (epoch > 25000) and (epoch <= 50000):
                a3c.ENTROPY_WEIGHT = 0.5

            elif (epoch> 50000) and (epoch <= 80000):
                a3c.ENTROPY_WEIGHT = 0.3

            elif epoch > 80000 :
                a3c.ENTROPY_WEIGHT = 0.1

            print("ENTROPY = ", a3c.ENTROPY_WEIGHT)
...
def agent(agent_id, all_cooked_time, all_cooked_bw, net_params_queue, exp_queue):
    ...
    with tf.Session() as sess, open(LOG_FILE + '_agent_' + str(agent_id), 'wb') as log_file:
        ...
        while True:  # experience video streaming forever
            ...
            end_of_video, video_chunk_remain, video_score = \
                net_env.get_video_chunk(bit_rate)

            time_stamp += delay  # in ms
            time_stamp += sleep_time  # in ms

            scaled_video_score = video_score * 2 + 1
            # -- linear reward --
            # reward is video quality - rebuffer penalty - smoothness
            reward = scaled_video_score * VIDEO_BIT_RATE[bit_rate] / M_IN_K \
                     - REBUF_PENALTY * rebuf \
                     - SMOOTH_PENALTY * scaled_video_score * np.abs(VIDEO_BIT_RATE[bit_rate] -
